...
from .CameraInfo import CameraInfo, StereoCameraInfo, StereoInfo
...
class Calibrator:
    ...
    def next(
        ...
        left_cv = self.__pil_to_cv(left_image)
        right_cv = self.__pil_to_cv(right_image)
    ...
    def __cv_to_pil(self, cv_image: MatLike):
    ...
    def _process_image_pair(self, left_cv: MatLike, right_cv: MatLike):
        ...
        if ret_left and ret_right:
            ...
            self.left_image_with_drawing.on_next(self.__cv_to_pil(img_left))
            self.right_image_with_drawing.on_next(self.__cv_to_pil(img_right))
            return True

        self.left_image_with_drawing.on_next(self.__cv_to_pil(left_cv))
        self.right_image_with_drawing.on_next(self.__cv_to_pil(right_cv))
    ...
    def update_calibration(self):
        ...
        (
            ...
            _rvecs_left,
            _tvecs_left,
        ) = cv2.calibrateCamera(  # pyright: ignore[reportCallIssue]
            self.objpoints,
            self.imgpoints_left,
            self.img_shape,
