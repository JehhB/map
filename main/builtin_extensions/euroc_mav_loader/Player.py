...
class PlayerEvent(BaseEvent, Generic[T]):
    def __init__(self, type: str, player: "Player[T]", state: PlayerState) -> None:
        super().__init__(type, player, state)


class Player(EventTarget, Generic[T]):
    ...
    def start(self):
        ...
        with self._lock:
            ...
            self.emit(PlayerEvent("player.start", self, "playing"))
    ...
    def pause(self):
        ...
        with self._lock:
            ...
            self.emit(PlayerEvent("player.pause", self, "paused"))
    ...
    def resume(self):
        ...
        with self._lock:
            ...
            self.state_subject.on_next("playing")
            self.emit(PlayerEvent("player.resume", self, "playing"))
    ...
    def stop(self):
        ...
        with self._lock:
            ...
            self.emit(PlayerEvent("player.stop", self, "idle"))
            return True
    ...
    def step(self):
        ...
        try:
            ...
            self.emit(PlayerEvent("player.completed", self, "completed"))
        ...
        try:
            ...
            while not self._stop_requested:
                ...
                if self._paused:
                    ...
                    _ = self._frame_rate_changed.wait(0.1)
                    self._frame_rate_changed.clear()
                    continue
