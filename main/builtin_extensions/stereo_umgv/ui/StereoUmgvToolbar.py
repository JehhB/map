...
from tkinter_rx import Button, ImageLabel, ImageObservable, LabelFrame
...
class StereoUmgvToolbar(tk.Frame):
    ...
    def __init__(
        ...
        joystick.grid(row=0, column=0, padx=4, pady=4, rowspan=7)

        _ = joystick_frame.columnconfigure(0, weight=1)
        joystick_frame.grid(row=2, column=0, sticky="ew", pady=24, columnspan=2)

        connect = Button(self, text="Connect", clickevent="stereo_umgv.connect")
        connect.event_target.parent = self.event_target
        connect.grid(row=3, column=0, sticky="ew", pady=2, padx=2)
        ...
        start_mapping.grid(row=3, column=1, sticky="ew", pady=2, padx=2)
